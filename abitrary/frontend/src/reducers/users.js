...
export default function users(state = reducerUtils.initial([]), action) {
  switch (action.type) {
    case types.GET_USERS:
      return reducerUtils.loading(state.users?.data);
    ...
    case types.CREATE_USER_SUCCESS:
      return reducerUtils.success([...state.users.data, action.payload]);
    case types.CREATE_USER_ERROR:
      return reducerUtils.error(action.payload, state.users.data);
    default:
      return state;
  }
}
